...
<?php ob_start();  ?>
        <?php 
            $insert_payments   = $this->db->get('payment')->result_array();
                ...
                <br>
            <?php
                // Définir le fuseau horaire pour obtenir les heures correctes
                ...
                // Si l'heure actuelle est avant 9h, prendre la période de 24h précédente
                ...
                $startDateTimeFormatted = $startDateTime->format('Y-m-d');
                $endDateTimeFormatted = $endDateTime->format('Y-m-d');

                // Exécuter une requête pour récupérer les rendez-vous dans la période de 24 heures
                $appointments = $this->db->query("SELECT COUNT(*) AS total FROM appointment 
                    WHERE STR_TO_DATE(date_timestamp, '%d-%m-%Y') 
                    BETWEEN '$startDateTimeFormatted' AND '$endDateTimeFormatted'")->row()->total;

            ?>
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                <a class="dashboard-stat bg-primary" href="<?php echo base_url(); ?>doctor/appointment">
                    <span class="name"><?php echo get_phrase('total-rendez-vous-24h') ?></span>
                    <span class="number counter"><?php echo $appointments; ?></span>
                    <span class="bg-icon"><i class="fa fa-dashboard"></i></span>
                </a>
            </div>
            ...
            $exams = $this->db->query("SELECT * FROM examen 
                WHERE STR_TO_DATE(creation_time, '%d-%m-%Y %H:%i:%s') 
            ...
            ?>
            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                <a class="dashboard-stat bg-primary" href="<?php echo base_url(); ?>doctor/examen">
                    <span class="name"><?php echo get_phrase('total-examens-24h') ?></span>
                    <span class="number counter"><?php echo ($totalExamens); ?></span>
                    <span class="bg-icon"><i class="fa fa-flask"></i></span>
                </a>
            </div>
            ...
            <?php
                ...
                // Formater les dates pour les requêtes SQL
                $startDateTimeFormatted = $startDateTime->format('Y-m-d H:i');
                $endDateTimeFormatted = $endDateTime->format('Y-m-d H:i');

                // Exécuter une requête pour récupérer le nombre total de prescriptions dans la période de 24 heures
                $prescriptionsCount = $this->db->query("SELECT COUNT(*) AS total FROM prescription 
                    WHERE STR_TO_DATE(prescription_timestamp, '%d-%m-%Y %H:%i') 
                    BETWEEN '$startDateTimeFormatted' AND '$endDateTimeFormatted'")->row()->total;
            ?>

            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                <a class="dashboard-stat bg-primary" href="<?php echo base_url(); ?>doctor/prescription">
                    <span class="name"><?php echo get_phrase('total-prescriptions-24h') ?></span>
                    <span class="number counter"><?php echo $prescriptionsCount; ?></span>
