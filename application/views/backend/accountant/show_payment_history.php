...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
 ...
 <a class="dashboard-stat bg-info" href="#">
    <span class="name"><?php echo get_phrase('Montants total') ?></span><br>
    <span class="number counter">
    	<?php 
           $test = "Vous allez bien j'espère"; 
           echo $test;
    	 ?> <br><br>
    	<?php 
...
$query = $this->db->query("SELECT SUM(net_amount) AS total FROM invoice WHERE creation_datetime BETWEEN '$start_date' AND '$end_date'");
$result = $query->row();

if ($result) {
    $total = $result->total;
    echo "Le montant total des reçus créés entre $start_date et $end_date est : $total";
} else {
    echo "Aucun résultat trouvé pour cette période de dates.";
}
?>
						...
						<thead>
						    ...
						    <tbody>			
								...
								<tr>
									...
									<td><?php echo $row['creation_datetime']; ?>
									</td>
									<td><?php echo $row['due_timestamp']; ?>
									</td>
									
									<td><?php echo $row['net_amount'] ?></td>
									<td><?php echo $row['status'] ?></td>
									<td>
										<a  href="<?php echo base_url(); ?>InvoiceComptable/invoice_print/<?php echo $row['invoice_id']; ?>" 
