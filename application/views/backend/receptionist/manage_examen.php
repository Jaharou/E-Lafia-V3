...
<div class="col-md-12">
    <div class="panel">
        <a href="<?php echo base_url(); ?>receptionist/examen_crud/add"
        ...
        <div class="panel-body p-20">
            <table id="example" class="display table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                    <?php
                    // Liste des examens de la fenêtre en cours (du plus récent
                    // au plus ancien). Fenêtre 9h-9h : la liste se réinitialise
                    // chaque jour à 9h du matin (comportement historique
                    // demandé). La jointure reste en LEFT JOIN pour qu'un
                    // examen dont le patient aurait été supprimé ne disparaisse
                    // pas silencieusement.
                    ...
                    $this->db->select('examen.*, patient.name AS patient_name, patient.prenom');
                    $this->db->from('examen');
                    $this->db->join('patient', 'patient.patient_id = examen.patient_id', 'left');
                    $this->db->where('examen.creation_time >=', $window_start->format('Y-m-d H:i:s'));
                    $this->db->where('examen.creation_time <', $window_end->format('Y-m-d H:i:s'));
                    $this->db->order_by('examen.id_examen', 'DESC');
                    ...
                    foreach ($exams as $examen) {
                        ...
                        $name = !empty($examen['patient_name']) ? $examen['patient_name'] : 'Patient';
                        $prenom = !empty($examen['prenom']) ? $examen['prenom'] : 'non renseigné';

                        $rid = $examen['receptionist_id'];
                        ...
                        echo "<tr>
                                ...
                                <td>" . htmlspecialchars($examen['examen_number']) . "</td>
                                <td>" . htmlspecialchars($examen['creation_time']) . "</td>
                                <td>" . htmlspecialchars($name . " " . $prenom) . "</td>
                                <td>";

                        // Décoder et afficher les libellés des tests de l'examen
                        ...
                        if (is_array($examen_entries)) {
                            foreach ($examen_entries as $entry) {
                                $total_amount += isset($entry->amountT) ? (float) $entry->amountT : 0;
                            }
                        }
                        echo $total_amount;
                        echo "  </td>
                                <td>" . htmlspecialchars($examen['status']) . "</td>
                                <td>
                                    <a href=\"" . base_url() . "receptionist/examen_crud/edit/" . $examen['id_examen'] . "\" class=\"btn btn-success btn-rounded icon-only\" title=\"Modifier\">
                                        <i class=\"fa fa-pencil\" aria-hidden=\"true\"></i>
                                    </a>
                                    <a class=\"btn btn-warning btn-rounded icon-only\" href=\"" . base_url() . "InvoiceExamenCaisse/examenCaisse_print/" . $examen['id_examen'] . "\" title=\"Impression\">
...
<script>
    ...
    // (ORDER BY id_examen DESC) ; plus besoin d'inverser les lignes côté
