...
<div class="row">
    <div class="col-md-8" style="left: 180px;">
        <div class="panel panel-primary">
             <a href="<?php echo base_url(); ?>receptionist/vente" class="btn bg-black btn-wide pull-left"><i class="fa fa-arrow-left"></i> <?php echo get_phrase('retour'); ?></a>
            <div style="clear:both;"></div>                     
            <div class="panel-body p-70">
            <form method="post" action="<?php echo base_url(); ?>receptionist/vente/update/<?php echo $row['vente_id']; ?>" class="p-70" id="form-validate" enctype="multipart/form-data">                
                <div class="row">       
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="vente_number"  value="<?php echo $row['vente_number']; ?>"  readonly>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <br>
                    <span style="color: green; font-size: 25px;">Espace ventes</span>
                    ...
                    <div id="vente_entry_temp" style="display:none;">
                        <div class="form-group vente_entry">
                            <label for="vente" class="col-sm-3 control-label">
                                <?php echo get_phrase('nouvelle_vente'); ?></label>
                            <div class="col-sm-2">       
                                <select name="medicine_id[]" class="form-control select2 name" id="stock_id">
                                    <optgroup>
                            ...
                            <div class="col-sm-2">
                                <input type="text" class="js-states form-control amount pu" name="pu[]" value="" placeholder="<?php echo get_phrase('pu'); ?>" id="prix" readonly>
                            </div>
                            <div class="col-sm-2">
                            <input type="text" class="js-states form-control qte_vente quantite" id="qte_vente" name="quantite[]" placeholder="<?php echo get_phrase('quantité'); ?>">
                            </div>
                            <div class="col-sm-2">
                                <input type="text" class="js-states form-control Ptotal prixT" id="pT" name="prixT[]" value="0" placeholder="<?php echo get_phrase('montant_total'); ?>" readonly>
                            </div>
                            <div class="col-sm-1">
                    ...
                    <div id="vente_entry">
                        ...
                        $invoice_ventes = json_decode($row['invoice_ventes']);
                            ...
                            <div class="form-group">
                               ...
                               <?php echo get_phrase('vente'); ?></label>
                                <div class="col-sm-2">
                                    <?php 
                                    // Initialiser le JSON 'invoice_ventes'
                                    $invoice_ventes = json_decode($row['invoice_ventes']);
                                    foreach ($invoice_ventes as $vente_entry) {
                                        $medicine_id = $vente_entry->produit;
                                        
                                        // Requête pour récupérer le nom du produit par son ID
                                        $this->db->select('name');
                                    ...
                                    <!-- Champ de texte pour afficher le nom du produit à l'utilisateur (non modifiable) -->
                                    <input type="text" class="js-states form-control" value="<?php echo $name; ?>" readonly>
                                    
                                    <?php } ?>
                                </div>
                                <div class="col-sm-2">
                                    <input type="text" class="js-states form-control pu amount" name="pu[]"  
                                    value="<?php echo $vente_entry->amount; ?>" >
                                </div>
                                <div class="col-sm-2">
                                <input type="text" class="js-states form-control qte_vente quantite" name="quantite[]" value="<?php echo $vente_entry->qte_vente; ?>" >
                                </div>
                                <div class="col-sm-2">
                                    <input type="text" class="js-states form-control Ptotal prixT" name="prixT[]" value="<?php echo $vente_entry->Ptotal; ?>" >
                                </div>
                                <div class="col-sm-1">
    ...
    function add_entry() {
        $("#vente_entry").append(blank_vente_entry);
    }
    ...
    // Affiche de pu en sélectionnant son produit et calcul de prix total
    document.addEventListener('DOMContentLoaded', function () {
    // Add event listener to dynamically added elements
    $(document).on('change', '.name', function () {
        const selectedOption = this.options[this.selectedIndex];
        const price = selectedOption.getAttribute('data-price');
        $(this).closest('.vente_entry').find('.amount').val(price ? price : '');
    ...
    function calculateTotalPrice(element) {
        const venteEntry = $(element).closest('.vente_entry');
        const price = parseFloat(venteEntry.find('.amount').val()) || 0;
        const quantity = parseFloat(venteEntry.find('.quantite').val()) || 0;
        const totalPrice = price * quantity;
        venteEntry.find('.prixT').val(totalPrice.toFixed(0));
        }
    });
