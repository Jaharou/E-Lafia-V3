...
class JournalVenteAdmin extends CI_Controller {
    ...
    public function journalAdmin_vente_print() {
        if ($this->session->userdata('receptionist_login') == 1 || $this->session->userdata('admin_login') == 1) {
            ...
            // Récupération des informations des ventes pour une date de vente spécifique
$this->db->select('vente.*, patient.name AS name, patient.prenom AS prenom, receptionist.name AS receptionist_name');
$this->db->from('vente');
$this->db->join('patient', 'patient.patient_id = vente.patient_id');
$this->db->join('receptionist', 'receptionist.receptionist_id = vente.receptionist_id', 'left'); // Joindre la table receptionist

// Conversion du format de la date_vente de 'd-m-Y' à 'Y-m-d' avant d'appliquer le filtre
if (!empty($date_vente)) {
    $formatted_date = date('Y-m-d', strtotime($date_vente)); // Convertir la date au format MySQL
    $this->db->where('DATE(vente.date_vente)', $formatted_date); // Filtrer par date_vente
}

// Appliquer un filtre pour le réceptionniste si spécifié
if ($receptionist_id) {
    $this->db->where('vente.receptionist_id', $receptionist_id);
}
...
$vente_info = $this->db->get()->result_array();
            ...
            function header_value($pdf, $system_name, $contact, $address, $system_email, $system_nif) {
                ...
                $pdf->SetY(-40);
                $pdf->SetFont('Arial', 'B', 12);
                $pdf->Cell(0, 10, 'Montant total des ventes: ' . $totalPtotal .' FCFA ', ',', ' ' . ' ' . $currency_symbol, 0, 1, 'R');
