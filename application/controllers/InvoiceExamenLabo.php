...
class InvoiceExamenLabo extends CI_Controller {
    ...
    /*     * ****PRINT INVOICE**** */
    public function examenLabo_print($param1 = ''){ 
        if ( $this->session->userdata('laboratorist_login') == 1 || $this->session->userdata('admin_login') == 1 ):
			$user_id = $this->session->userdata('login_user_id');
			if( $this->session->userdata('laboratorist_login') == 1 ){
				$athorised_email = $this->db->get_where('laboratorist', array('laboratorist_id' => $user_id))->row()->name;
			}else{				
            ...
            $this->CreateTable_row($examenentry->qte_examen, utf8_decode($examenentry->resultat), utf8_decode($examenentry->unite), utf8_decode($examenentry->description), utf8_decode($examenentry->intervalle));
             }
    ...
    public function CreateTable_row($qte_examen = '', $resultat = '', $unite = '', $examen_entries = '', $intervalle = '') {
    ...
    $this->fpdf->Cell(0, 6, utf8_decode("Biologiste: ") . utf8_decode($atho), 0, 0, 'R');
    $this->fpdf->Ln(5);
    }
