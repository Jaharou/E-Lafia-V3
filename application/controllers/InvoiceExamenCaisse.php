...
class InvoiceExamenCaisse extends CI_Controller {
    ...
    /*     * ****PRINT INVOICE**** */
    public function examenCaisse_print($param1 = ''){ 
        if ( $this->session->userdata('receptionist_login') == 1 || $this->session->userdata('admin_login') == 1 ):
            ...
            $examen_entries    = json_decode($examen_entry);    
             ...
             foreach ($examen_entries as $examenentry){
                 $i++;
                $total_amount += intval($examenentry->montant);
                $examen_entries = $examenentry->description;
                $examen_entries = $examenentry->resultat;
                $currency_symbol . $examenentry->montant;
            ...
            $this->examen_numero( utf8_decode("Reçu de paiement N°"). $examen_number);
            $this->fpdf->Ln(6);
            ...
            $examen_entries    = json_decode($examen_entry);
            foreach ($examen_entries as $examenentry) {
            $this->CreateTable_row($examenentry->montant, $examenentry->qte_examen, utf8_decode($examenentry->resultat), '', utf8_decode($examenentry->description), $examenentry->amountT);
             }
    ...
    public function CreateTable_row($montant = '', $qte_examen = '', $resultat = '', $discount_amount = '', $examen_entries = '', $amountT = '') {
        $this->fpdf->SetFillColor(255);
        $this->fpdf->SetTextColor(0);
        $this->fpdf->SetFont('times', '', 10);
        $this->fpdf->Cell(50, 5, $examen_entries, 1, 0, 'L', true);
        $this->fpdf->Cell(20, 5, $montant, 1, 0, 'C', true);
        $this->fpdf->Cell(10, 5, $qte_examen, 1, 0, 'C', true);
        $this->fpdf->Cell(30, 5, $resultat, 1, 0, 'C', true);
        $this->fpdf->Cell(25, 5, $amountT, 1, 0, 'C', true);
