...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        else if ($this->session->userdata('magasin_login') == 1)
            redirect(base_url() . 'magasin', 'refresh');
    ...
    function validate_login($name = '', $password = '') {
        ...
        $query = $this->db->get_where('magasin', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('magasin_login', '1');
            $this->session->set_userdata('login_user_id', $row->magasin_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'magasin');
            return 'success';
        }
