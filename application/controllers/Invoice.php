...
class Invoice extends CI_Controller {
    ...
    public function invoice_print($param1 = ''){ 
        if ( $this->session->userdata('receptionist_login') == 1 || $this->session->userdata('admin_login') == 1 ):
            ...
            $creation_datetime = $this->db->get_where('invoice', array('invoice_id' => $param1))->row()->creation_datetime;
            ...
            $avance = $this->db->get_where('invoice', array('invoice_id' => $param1))->row()->avance; 
            ...
            $invoice_entries    = json_decode($invoice_entry);	
			 ...
			 foreach ($invoice_entries as $invoiceentry){
				 $i++;
				$total_amount += intval($invoiceentry->amount);
            ...
            $this->invoice_numero( utf8_decode('Reçu de paiement N°'). $invoice_number);
            $this->fpdf->Ln(6);
            ...
            $invoice_entries    = json_decode($invoice_entry);
            foreach ($invoice_entries as $invoiceentry) {
            $this->CreateTable_row($invoiceentry->net_amount, '', $invoiceentry->net_amount, utf8_decode($invoiceentry->description),  $invoiceentry->qte_consult);
             }

             $this->CreateTable_header($currency_symbol);
            $examen    = json_decode($invoice_entry);
            foreach ($examen as $invoiceentry) {
            $this->CreateTable_row($invoiceentry->amount, '', $invoiceentry->net_amount, utf8_decode($invoiceentry->description), $invoiceentry->qte_consult);
             }
            ...
            $this->header_value('', $system_name, 'logo', $contact, $address, $system_email, utf8_decode('Reçu de paiement N°'). $invoice_id, $system_nif);
            $this->fpdf->Ln(6);
    ...
    public function invoice_numero($invoice_number = '' ) {
        $this->fpdf->SetTextColor(0);
        $this->fpdf->SetFont('times', 'B', 12);
        $this->fpdf->Cell(60, -12,$invoice_number, 1, 0, 'L');
    }
    ...
    public function CreateTable_row($qte_consult = '', $pourcentage_prise = '', $discount_amount = '', $invoice_entries = '', $net_amount = '') {
        $this->fpdf->SetFillColor(255);
        $this->fpdf->SetTextColor(0);
        $this->fpdf->SetFont('times', '', 10);
        $this->fpdf->Cell(65, 5, $invoice_entries, 1, 0, 'L', true);
        //$this->fpdf->Cell(65, 5, $examen, 1, 0, 'L', true);
        $this->fpdf->Cell(25, 5, $qte_consult, 1, 0, 'C', true);
        $this->fpdf->Cell(10, 5, $net_amount, 1, 0, 'C', true);
        $this->fpdf->Cell(35, 5, $discount_amount, 1, 0, 'C', true);
        //$this->fpdf->Cell(30, 6, $amount ."", 1, 0, 'C', true);
        $this->fpdf->Ln();
    }

    public function invioce_footer_amount($net_amount = '') {
        //$this->fpdf->setFillColor(255,201,120);
        $this->fpdf->SetFont('times', 'B', 11);
        $this->fpdf->Cell(100, 5, utf8_decode("Montant total"), 1, 0, 'L', true);
        $this->fpdf->Cell(35, 5, $net_amount . "", 1, 0, 'R', true); 
        $this->fpdf->Ln(6);  
    }
    ...
    public function invioce_footer_atho($atho = '' ) {
        ...
        $this->fpdf->SetFont('times', 'B', 10);
        $this->fpdf->SetY(-27);
        strlen($this->fpdf->Cell(135, 6, utf8_decode("Le caissier: ").utf8_decode($atho)."", 0, 0, 'R'));
    }
