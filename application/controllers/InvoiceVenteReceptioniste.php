...
class InvoiceVenteReceptioniste extends CI_Controller {
    ...
    public function vente_print($param1 = '') { 
        if ($this->session->userdata('receptionist_login') == 1 || $this->session->userdata('admin_login') == 1) {
            ...
            foreach ($invoice_ventes as $venteentry) {
                ...
                $this->CreateTable_row($venteentry->amount, '', $venteentry->qte_vente, utf8_decode($venteentry->produit), $venteentry->Ptotal);
            }
            ...
            $this->fpdf->Ln(5);
            $this->fpdf->Ln();
            $this->header_value('', $system_name, 'logo', $contact, $address, $system_email, utf8_decode('Reçu pharmacie N°') . $vente_number, $system_nif);
            $this->fpdf->Ln(6);
    ...
    public function CreateTable_row($qte_vente = '', $pourcentage_prise = '', $amount = '', $invoice_ventes = '', $Ptotal = '') {
    ...
    public function invioce_footer_atho($atho = '') {
        ...
        $this->fpdf->Cell(0, 6, utf8_decode("Le réceptioniste: ") . utf8_decode($atho), 0, 0, 'R');
        $this->fpdf->Ln(5);
        }
    }
