...
function invoice_manage($task = "", $invoice_id = "") {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'doctor/invoice_manage');
        }
    ...
    function consultation_crud($task = "", $param2 = "", $param3 = ""){
        if ($this->session->userdata('doctor_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
        }
        if ($task == 'add') {
            $page_data['page_name'] = 'add_consultation';
            $page_data['page_title'] = get_phrase('Consultation');
            $this->load->view('backend/index', $page_data);
        }elseif ($task == 'edit') {
           $page_data['page_name'] = 'edit_consultation';
           $page_data['param2'] = $param2;
           $page_data['param3'] = $param3;
           $page_data['page_title'] = get_phrase('Consultation');
           $this->load->view('backend/index', $page_data);
    ...
    function examen($task = "", $id_examen = "") {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'doctor/examen');
        }
    ...
    //-------------------Fin de la partie examen---------------------------

    function type_crud($task = "", $param2 = "", $param3 = ""){
        if ($this->session->userdata('doctor_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
        }
        if ($task == 'add') {
            $page_data['page_name'] = 'manage_type';
            $page_data['page_title'] = get_phrase('ordonnance');
            $this->load->view('backend/index', $page_data);
        }elseif ($task == 'edit') {
           $page_data['page_name'] = 'type';
           $page_data['param2'] = $param2;
           $page_data['param3'] = $param3;
           $page_data['page_title'] = get_phrase('ordonnance_type');
           $this->load->view('backend/index', $page_data);
        }
    }
    ...
    function medicine($task = "", $medicine_id = "")
            ...
            if ($task == "create")
            {
                ...
                $this->session->set_flashdata('message' , get_phrase('informations_a_été_enregistré_avec_succés'));
                redirect(base_url() .'doctor/medicine');
            }
    ...
    function consultation($task = "", $consultation_id = "", $menu_check = '', $patient_id = '') {
        if ($this->session->userdata('doctor_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
        }
        ...
        if ($task == "create") {
            $this->crud_model->save_consultation_info();
            $this->session->set_flashdata('message', get_phrase('informations a-été-enregistrées-avec-succès'));
            redirect(base_url() . 'doctor/consultation');
        }

        if ($task == "update") {
            $this->crud_model->update_consultation_info($consultation_id);
            $this->session->set_flashdata('message', get_phrase('informations a-été-mise-à-jour-avec-succès'));
            if ($menu_check == 'from_consultation')
                redirect(base_url() . 'doctor/consultation');
        }

        if ($task == "delete") {
            $this->crud_model->delete_consultation_info($consultation_id);
            if ($menu_check == 'from_consultation')
                redirect(base_url() . 'doctor/consultation');
            else
                redirect(base_url() . 'doctor/consultation/' . $consultation_id);
        }

        $data['consultation_info'] = $this->crud_model->select_consultation_info_by_doctor_id();
        $data['menu_check'] = 'from_consultation';
