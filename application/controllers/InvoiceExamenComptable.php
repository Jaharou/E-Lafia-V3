...
class InvoiceExamenComptable extends CI_Controller {
    ...
    /*     * ****PRINT INVOICE**** */
    public function examenComptable_print($param1 = ''){ 
        if ( $this->session->userdata('accountant_login') == 1 || $this->session->userdata('admin_login') == 1 ):
            $user_id = $this->session->userdata('login_user_id');
            if( $this->session->userdata('accountant_login') == 1 ){
                $athorised_email = $this->db->get_where('accountant', array('accountant_id' => $user_id))->row()->name;
            }else{              
            ...
            $discount_amount = $this->db->get_where('examen', array('id_examen' => $param1))->row()->discount_amount;       
            $prise_en_charge = $this->db->get_where('examen', array('id_examen' => $param1))->row()->prise_en_charge;
            $mode_paiement = $this->db->get_where('examen', array('id_examen' => $param1))->row()->mode_paiement;
            $pourcentage_prise = $this->db->get_where('examen', array('id_examen' => $param1))->row()->pourcentage_prise;
            ...
            $examen_entries    = json_decode($examen_entry);    
             ...
             foreach ($examen_entries as $examenentry){
                 $i++;
                $total_amount += $examenentry->amount;
                $examen_entries = $examenentry->description;
                $examen_entries = $examenentry->resultat;
                $currency_symbol . $examenentry->amount;
                
            }
            
            $total_amount  = $this->crud_model->calculate_total_amount_examen($examen_number);
            $remise_amount = $this->crud_model->calculate_invoice_total_remise_amount_examen($examen_number);
            $prise_charge = $this->crud_model->calculate_total_prise_examen($examen_number); 
            $grand_total = $this->crud_model->calculate_invoice_total_amount_examen($examen_number);
            ...
            $examen_entries    = json_decode($examen_entry);
            foreach ($examen_entries as $examenentry) {
            $this->CreateTable_row($examenentry->amount, '1', utf8_decode($examenentry->resultat), '', utf8_decode($examenentry->description), $examenentry->amount);
             }
             $this->invioce_footer_amount_examen($total_amount .'  FCFA   ');
             $this->fpdf->Ln();

            $this->invioce_footer_examen($grand_total .'  FCFA', $currency_symbol, $remise_amount .'  FCFA', $prise_charge .'  FCFA');
            $this->fpdf->Ln(5);
            ...
            $this->header_value('', $system_name, 'logo', $contact, $address, utf8_decode("Reçu de paiement N°"). $examen_number, $system_nif);
             $this->fpdf->Ln(6);
    ...
    public function header_value($barcode_text = '', $title_tab = '', $logo = '', $contact = '', $address = '', $mail_title = '',$system_nif) {
        ...
        $x = 75; // Coordonnée X de départ
        $y = 30; // Coordonnée Y de départ
        $width = 68; // Largeur de chaque cellule
    ...
    public function invioce_footer_examen($gtotal = '', $currency = '', $remise_amount ='', $prise_charge = '') {
        //$this->fpdf->setFillColor(255,201,120);
        $x = 1; 
        $y = 122;
        $width = 0;
    ...
    public function CreateTable_header($currency = '') {
        ...
        $this->fpdf->Ln(1);
        $this->fpdf->Cell(0, 5, utf8_decode('Résultat des analyses médicales'),0,1,'C');
        $this->fpdf->Cell(50, 5, "Test"."", 1, 0, 'L', true);
    ...
    public function CreateTable_row($amount = '', $qte = '', $resultat = '', $discount_amount = '', $examen_entries = '') {
        $this->fpdf->SetFillColor(255);
        $this->fpdf->SetTextColor(0);
        $this->fpdf->SetFont('times', '', 10);
        $this->fpdf->Cell(50, 5, $examen_entries, 1, 0, 'L', true);
    ...
    public function invioce_footer_amount_examen($total_amount = '') {
        //$this->fpdf->setFillColor(255,201,120);
    ...
    public function invioce_footer_atho($atho = '' ) {
        ...
        $this->fpdf->Ln(20);
        $this->fpdf->SetFont('times', 'B', 10);
        strlen($this->fpdf->Cell(135, 6, utf8_decode("Le comptable: ").utf8_decode($atho)."", 0, 0, 'R'));
    }
